...
class OutdatedSubState extends MusicBeatState
{
	...
	{
		super.create();
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('Disclaimer', 'shared'));
		bg.scale.x *= 1;
		bg.scale.y *= 1;
		bg.screenCenter();
		add(bg);
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"Thank for Downloading Vs Screwy DEMO"
			+ "\nIt is recommended to play without maximized window"
			+ "\nThis is only for Cutscenes in Story-mode so they play properly"
			+ "\n"
			+ "\n"
			+ "\nAny Feedback is welcome!"
			+ "\nBe patient for the full release!!!",
			32);
