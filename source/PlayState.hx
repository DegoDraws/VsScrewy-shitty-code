...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			{
				defaultCamZoom = 0.8;
				curStage = 'screwyplace';
				var bg:FlxSprite = new FlxSprite(-260, -70).loadGraphic(Paths.image('fondo'));
				bg.antialiasing = true;
			...
			{
				defaultCamZoom = 0.8;
				curStage = 'screwyplacelo';
				var bg:FlxSprite = new FlxSprite(-260, -70).loadGraphic(Paths.image('fondox2'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'stage':
				{
						defaultCamZoom = 0.9;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-350, -137).loadGraphic(Paths.image('stageback'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		switch(curStage){
		case 'screwyplace': bfversion = 'bfjacket';}
		boyfriend = new Boyfriend(770, 450, bfversion);
		switch(curStage){
		case 'screwyplacelo': bfversion = 'bfjacketlo';}
		boyfriend = new Boyfriend(770, 450, bfversion);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " - DEMO VERSION "  : ""), 16);
	...
	function muebles(muebles:Int = 0)
		{
			var imagen:FlxSprite = new FlxSprite(0,0);
			imagen.loadGraphic(Paths.image('Overlayv'));
	...
	function muebled(muebled:Int = 0)
		{
			var imagen:FlxSprite = new FlxSprite(0,0);
			imagen.loadGraphic(Paths.image('Overlayv22'));
	...
	function mueblex(mueblex:Int = 0)
		{
			var imagen:FlxSprite = new FlxSprite(0,0);
			imagen.loadGraphic(Paths.image('postes'));
	...
	{
		if (curStage =='screwyplace')
			{
			 muebles(1);
			};
		if (curStage =='screwyplacelo')
			{
			 muebled(1);
			};
		if (curStage =='stage')
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (FlxG.save.data.scoreScreen){
						switch (cancion.toLowerCase())
					{
						case 'tutorial':
						var video:VideoHandlerMP4 = new VideoHandlerMP4();
						video.playMP4(Paths.video('cs'), new StoryMenuState());
						default:
				...
				else
				{
					...
					switch (cancion.toLowerCase())
					{
						...
						FlxG.camera.alpha = 0;
						var video:VideoHandlerMP4 = new VideoHandlerMP4();
                        video.playMP4(Paths.video('cs2'), new PlayState());
					}
					if (curStage == 'screwyplacelo')
						{
							FlxG.camera.alpha = 1;
